...
export default function DashboardHome() {
  ...
  const monthLabel = useMemo(
    () =>
      new Date().toLocaleDateString(locale === 'ko' ? 'ko-KR' : 'en-US', { year: 'numeric', month: 'long' }),
    [locale]
  );
    ...
    <div className="w-full space-y-8">
      ...
      >
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h2 className="text-xl sm:text-2xl font-extrabold text-slate-100">{t('overview')}</h2>
            <p className="text-sm sm:text-base text-slate-500 mt-1">{t('overviewSubtitle')}</p>
          </div>
      ...
      <section className="p-4 md:p-6 rounded-2xl w-full" style={{ background: cardBg, border: '1px solid rgba(255,255,255,0.03)' }}>
        <h3 className="text-lg sm:text-xl font-semibold text-slate-100">{t('taskActivity')}</h3>
        <p className="text-sm sm:text-base text-slate-500 mt-1">{t('taskActivitySubtitle')}</p>
  ...
  return (
    <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-12 lg:items-stretch">
      <div className="lg:col-span-4">
        <StatsCard
          title={t('stats.remainingQueue')}
          value={balance === null ? t('stats.loading') : t('stats.queueUnits', { balance })}
          subtitle={t('stats.balanceSubtitle')}
        />
      </div>
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="text-sm text-slate-400">{t('stats.monthUsage')}</div>
          <div className="text-xs text-slate-500">
            {t('stats.unitsLabel')}
            {monthUsageTotal > 0 ? t('stats.totalUsage', { total: monthUsageTotal }) : ''}
          </div>
        </div>
...
function TaskActivityList({ tasks, tasksLoading }: { tasks: any[]; tasksLoading: boolean }) {
  ...
  if (tasks.length === 0) {
    return <div className="py-8 text-center text-slate-500">{t('tasks.empty')}</div>;
  }

  return (
    <>
      {tasks.map((tRow) => (
        <TaskRow key={tRow.id} task={tRow} />
      ))}
    </>
  );
}

function TaskRow({ task: tRow }: { task: UsageHistoryRow }) {
  const t = useTranslations('dashboardHome');
  const burned = burnedQueueAmount(tRow);
  const status = (tRow.status || '').toLowerCase();
  const badge = (() => {
    if (status === 'queued') return { text: t('status.queued'), color: 'bg-yellow-500 text-black' };
    if (status === 'processing') return { text: t('status.processing'), color: 'bg-blue-500 text-white' };
    if (status === 'completed') return { text: t('status.completed'), color: 'bg-green-500 text-white' };
    if (status === 'rejected') return { text: t('status.rejected'), color: 'bg-red-500 text-white' };
    return { text: tRow.status || t('status.unknown'), color: 'bg-gray-500 text-white' };
  })();
    ...
    <div className="py-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-3">
          <div className="text-sm font-medium text-slate-100 truncate">{tRow.task_subject || t('taskFallback', { id: tRow.id })}</div>
          <div className={`text-xs px-2 py-1 rounded-full shrink-0 ${badge.color}`}>{badge.text}</div>
        </div>
        <div className="text-xs text-slate-500 mt-1">
          {t('tasks.sentTo')}: {tRow.customer_email} • {timeAgo(tRow.created_at)}
        </div>
        <div className="mt-2 text-xs font-medium" style={{ color: neon }}>
          {burned > 0 ? t('tasks.burned', { amount: burned.toLocaleString() }) : t('tasks.noBurn')}
        </div>
      </div>
      <div className="flex sm:flex-col items-center sm:items-end gap-2 shrink-0">
        <div className="text-xs text-slate-400">{new Date(tRow.created_at).toLocaleString()}</div>
        <TaskActionButton task={tRow} />
      </div>
    </div>
  );
}

function TaskActionButton({ task: tRow }: { task: UsageHistoryRow }) {
  const burned = burnedQueueAmount(tRow);
  const t = useTranslations('dashboardHome');
  return (
    <span className="text-sm text-slate-400 px-3 py-1 rounded border border-gray-800">
      {burned > 0 ? t('tasks.bfaxUnits', { amount: burned.toLocaleString() }) : t('tasks.dash')}
    </span>
  );
}
