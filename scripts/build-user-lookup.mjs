  ...
  const searchUser = async () => {
    ...
    try {
      ...
      try {
        ...
        setProfile(row);
        setMessage(
          'Vercel\uC5D0 SUPABASE_SERVICE_ROLE_KEY\uB97C \uC124\uC815\uD558\uBA74 \uC804\uCCB4 \uC720\uC800 \uC870\uD68C\uAC00 \uAC00\uB2A5\uD569\uB2C8\uB2E4.'
        );
      } catch (fallbackError) {
        ...
        setMessage(\`\uC720\uC800 \uC870\uD68C\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4. (\${detail})\`);
      }
    } finally {
      setLoading(false);
    }
...
const tail = [
  ...
  '          />',
  '        ' + close,
  '        <button type="button" onClick={searchUser} disabled={loading} className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#10b981]/40 bg-[#07160f] px-5 py-3 text-sm font-semibold text-[#10b981] transition hover:bg-[#0a2018] disabled:opacity-50">',
  '          <Search className="h-4 w-4" />',
  "          {loading ? '\uC870\uD68C \uC911\u2026' : '\uC720\uC800 \uAC80\uC0C9'}",
  '        </button>',
  '      ' + close,
  '      {message ? (',
  '        <div className="rounded-lg border border-zinc-800 bg-zinc-950/80 px-4 py-3 text-sm text-zinc-300">{message}' + close,
  '      ) : null}',
  '      {profile ? (',
  '        <motionless />',
  '      ) : null}',
...
const modal = [
  ...
  '              ' + close,
  '              <div><label className="text-xs text-zinc-500">BFAX \uC218\uB7C9</label><input type="number" min={1} value={adjustAmount} onChange={(e) => setAdjustAmount(e.target.value)} className="mt-1 w-full rounded-lg border border-zinc-800 bg-[#060606] px-3 py-2 text-sm text-slate-100" /></motionless>',
  ...
  '              <div><label className="text-xs text-zinc-500">\uBA54\uBAA8 (\uC7A5\uBD80)</label><input value={adjustNote} onChange={(e) => setAdjustNote(e.target.value)} placeholder="ADMIN_ADJUST \uC0AC\uC720" className="mt-1 w-full rounded-lg border border-zinc-800 bg-[#060606] px-3 py-2 text-sm text-slate-100" /></motionless>',
  '            ' + close,
  ...
  '          ' + close,
  '        ' + close,
].join('\n').replaceAll('</motionless>', close);

let out = head + tail;
out = out.replace('        <motionless />\n      ) : null}\n      {adjustOpen', profile + '\n      ) : null}\n      {adjustOpen');
out = out.replace('        <motionless />\n      ) : null}\n    ' + close, modal + '\n      ) : null}\n    ' + close);

fs.writeFileSync(target, out, 'utf8');
console.log('ok', fs.statSync(target).size);
